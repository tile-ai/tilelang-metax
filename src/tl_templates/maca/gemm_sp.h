// Copyright (c) 2026 MetaX Integrated Circuits (Shanghai) Co., Ltd. All rights
// reserved.

#include <mctlass/gemm/threadblock/default_mma_core_sparse_sm80.h>
...
template <typename Shape> struct ShapeCheck<mctlass::half_t, Shape> {
  static constexpr bool value =
      (Shape::kM % 32 == 0) && (Shape::kN % 32 == 0) && (Shape::kK % 32 == 0);
};

template <typename Shape> struct ShapeCheck<mctlass::bfloat16_t, Shape> {
  static constexpr bool value =
      ShapeCheck<mctlass::half_t, Shape>::value; // Same as half
};
...
template <> struct DispatchInstructionShape<mctlass::half_t> {
  using Shape = mctlass::gemm::GemmShape<16, 8, 32>;
  using Operator = mctlass::arch::OpMultiplyAdd;
};

template <> struct DispatchInstructionShape<mctlass::bfloat16_t> {
  using Shape = mctlass::gemm::GemmShape<16, 8, 32>;
  using Operator = mctlass::arch::OpMultiplyAdd;
};
...
// template<>
// struct DispatchInstructionShape<mctlass::tfloat32_t> {
//   using Shape = mctlass::gemm::GemmShape<16, 8, 16>;
//   using Operator = mctlass::arch::OpMultiplyAdd;
// };

template <> struct DispatchInstructionShape<int8_t> {
  using Shape = mctlass::gemm::GemmShape<16, 8, 64>;
  using Operator = mctlass::arch::OpMultiplyAddSaturate;
};
...
// TODO: Not supported for now
// template<>
// struct DispatchInstructionShape<mctlass::int4b_t> {
//   using Shape = mctlass::gemm::GemmShape<16, 8, 128>;
//   using Operator = mctlass::arch::OpMultiplyAddSaturate;
// };
...
template <> struct DispatchType<mctlass::half_t> {
...
public:
  ...
  using LayoutA =
      typename std::conditional_t<trans_A, mctlass::layout::ColumnMajor,
                                  mctlass::layout::RowMajor>;
  using LayoutB =
      typename std::conditional_t<trans_B, mctlass::layout::ColumnMajor,
                                  mctlass::layout::RowMajor>;
  ...
  using WarpShape = mctlass::gemm::GemmShape<ThreadblockShape::kM / num_warp_m,
  ...
  using Policy = mctlass::gemm::warp::MmaTensorOpPolicy<
      mctlass::arch::SparseMma<InstructionShape, 32, ElementA,
                               mctlass::layout::RowMajor, ElementB,
                               mctlass::layout::ColumnMajor, ElementC,
                               mctlass::layout::RowMajor, Operator>,
      mctlass::MatrixShape<1, 1>>;
  using MmaWarp =
      mctlass::gemm::warp::SparseMmaTensorOp<WarpShape, ElementA, SmemLayoutA,
  ...
  static_assert(std::is_same_v<SmemLayoutE, mctlass::layout::ColumnMajor>,
  ...
  using ShapeA = mctlass::MatrixShape<Shape::kM, Shape::kK / kSparse>;
  using ShapeB = mctlass::MatrixShape<Shape::kK, Shape::kN>;
  using ShapeE =
      mctlass::MatrixShape<Shape::kM * 2,
...
TL_DEVICE void gemm_sp_ss(A_type *pA, B_type *pB, C_type *accum, E_type *pE) {
  using MMA =
      GemmTensorOp<mctlass::gemm::GemmShape<M, N, K>, num_warp_m, num_warp_n,
