...
def __fns(mask: int | PrimExpr, base: int | PrimExpr, offset: int | PrimExpr) -> PrimExpr:
    ...
    if _IS_MACA_AVAILABLE:
        return tirx.call_intrin("uint64", tirx.op.Op.get("tl.__fns"), mask, base, offset)
...
def shfl_xor(
    ...
    """
    if _IS_MACA_AVAILABLE:
        return tirx.call_intrin(value.dtype, tirx.op.Op.get("tl.shfl_xor_sync"), _as_uint64_mask(mask), value, delta, width)
...
def shfl_down(
    ...
    """
    if _IS_MACA_AVAILABLE:
        return tirx.call_intrin(value.dtype, tirx.op.Op.get("tl.shfl_down_sync"), _as_uint64_mask(mask), value, delta, width)
...
def shfl_up(
    ...
    """
    if _IS_MACA_AVAILABLE:
        return tirx.call_intrin(value.dtype, tirx.op.Op.get("tl.shfl_up_sync"), _as_uint64_mask(mask), value, delta, width)
...
def shfl_sync(
    ...
    """
    if _IS_MACA_AVAILABLE:
        return tirx.call_intrin(value.dtype, tirx.op.Op.get("tl.shfl_sync"), _as_uint64_mask(mask), value, srcLane, width)
...
def maca_barrier_arrive_and_wait(barrier: tirx.Buffer | BufferLoad | PrimExpr) -> tirx.Stmt:
    """Perform MACA barrier arrive and wait operation.

    This is used to synchronize after memcpy_async operations.

    Args:
        barrier: tirx.Buffer | BufferLoad | PrimExpr
            The barrier handle (from T.alloc_maca_barrier()) or buffer load (e.g., bar[0]).

    Returns:
        tirx.Stmt: A statement representing the barrier_arrive_and_wait operation.
    ...
    """
    barrier = _mbar_to_buffer_load(barrier)
    return evaluate(tirx.call_intrin("void", tirx.op.Op.get("tl.maca_barrier_arrive_and_wait"), barrier))
