...
def setup_module_env():
    ...
    tvm_ffi.register_global_func("tilelang_callback_maca_postproc", f=lambda code, _: code, override=True)
...
def test_disk_cache_with_postproc(clean_cache_env, backend):
    ...
    Tests backends:
    (tvm_ffi, cython, torch use the same maca_postproc callback)
