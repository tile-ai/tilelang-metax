...
# and replace .cu** with .metax for Metax GPU
...
# if it's metax
docker run -it --cap-add=SYS_ADMIN --network=host --device=/dev/dri --device=/dev/mxcd --cap-add=SYS_PTRACE --shm-size=4G --security-opt seccomp=unconfined --security-opt apparmor=unconfined --name tilelang_test tilelang_workspace bash
```
